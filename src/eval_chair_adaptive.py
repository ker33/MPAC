...
# 超参数配置（多维协同优化版）
# ==========================================
ALPHA_T = 8  # 文本域联想头 (SAH) 的抑制强度 (进一步加大对恶性文本脑补头的封锁)
ALPHA_V = 8  # 跨模态域联想头 (VSAH) 的抑制强度 (进一步加大对恶性视觉脑补头的封锁)
BETA_T = 5      # 文本复制头 (ICH) 的正向辅助增强系数 (提升高视觉接地头在上下文中的语法一致性)
BETA_V = 12     # 视觉翻译头 (VTH) 的增强系数 (大幅度正向放大，强行拉高视觉注意力)
MIN_SCALE = 0.2  # 保底保留比例 (下调至 0.1，允许干预机制更干净地过滤脑补信号)
...
# ==========================================
# 步骤 5: 动态钩子注入 (Adaptive Dual-Steering Intervention)
# ==========================================
print("\n" + "="*50)
print("--- [ADAPTIVE INTERVENTION INITIALIZATION] ---")
print(f"Applying pre-hooks with ALPHA_T={ALPHA_T}, ALPHA_V={ALPHA_V}, BETA_V={BETA_V}, BETA_T={BETA_T}, MIN_SCALE={MIN_SCALE}")
...
for l in range(num_layers):
    ...
    for h in range(num_heads):
        ...
        # --- 策略分支 1：正向多维协同增强（正分头） ---
        # 如果是视觉翻译头 (VTH, 支配度得分为正)，直接通过 BETA_V 强力放大视觉注意力
        # 同时若其具有正向文本复制能力 (t_score > 0)，给予 BETA_T 的辅助增强以维持流畅度
        if v_score > 0:
        ...
        # 若在两个域中皆表现出负向偏置，对其施加联合加重惩罚，彻底阻断双重幻觉路径
        else:
...
print(f"Hook registration complete.")
...
output_path = "results/chair/chair_outputs_adaptive_b013.json"
