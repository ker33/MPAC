...
# ==========================================
# 介入超参数配置（完美对齐 5 参数多维优化逻辑）
# ==========================================
ALPHA_T = 8   # 文本域联想头 (SAH) 的抑制强度 (对齐 CHAIR 的 12.0 强力压制)
ALPHA_V = 8    # 跨模态域联想头 (VSAH) 的抑制强度 
BETA_T = 5      # 文本复制头 (ICH) 的辅助增强系数 (维护高可信度句子的语言连续性)
BETA_V = 12     # 视觉翻译头 (VTH) 的增强系数 (强行放大正确接地特征的决定比重)
MIN_SCALE = 0.2  # 保底保留比例 (下调至 0.1，实现极限脑补阻断)

# 控制评测样本量：None 代表全量 3000 个样本；可设为 300 等数值快速调试
LIMIT_SAMPLES = None 
...
# ==========================================
# 步骤 5: 动态钩子注入 (Adaptive Dual-Steering Setup)
# ==========================================
print("\n" + "="*50)
print("--- [POPE ADAPTIVE INTERVENTION SETUP] ---")
print(f"Injecting pre-hooks (ALPHA_T={ALPHA_T}, ALPHA_V={ALPHA_V}, BETA_V={BETA_V}, BETA_T={BETA_T}, MIN_SCALE={MIN_SCALE})")
...
for l in range(num_layers):
    ...
    for h in range(num_heads):
        ...
        # --- 策略分支 1：正向多维协同增强（正分头） ---
        # 若是视觉翻译头 (VTH, v_score > 0)，直接通过 BETA_V 强力放大视觉注意力
        # 同时若兼具正向文本复制能力 (t_score > 0)，给予 BETA_T 文本协调辅助以保持问答语义连贯
        if v_score > 0:
            boost_v = BETA_V * v_score
            boost_t = BETA_T * t_score if t_score > 0 else 0.0
            scale = 1.0 + boost_v + boost_t
            boosted_heads_count += 1
            
        # --- 策略分支 2：累加式联合脑补抑制（负分头） ---
        # 否则若为负偏置脑补头，施加多域联合累加惩罚，严密封锁臆断逻辑
        else:
...
print(f"Adaptive pre-hooks active.")
print(f"- Boosted Visual Translation Heads: {boosted_heads_count} heads")
print(f"- Suppressed Association Heads: {intervened_heads_count} heads")
print("="*50 + "\n")
...
for split in splits:
    ...
    # 保存单个子集的自适应结果 (后缀更新为 011 以标识新的双向干预版本)
    os.makedirs("results/pope", exist_ok=True)
    save_path = f"results/pope/pope_results_adaptive_{split}_b013.json"
