                    ...
                    if mask_to_apply.any():
                        for b_idx in range(bsz):
                            if mask_to_apply[b_idx]:
                                # 核心改进：直接将 logits 设为极小值。这在随后的 softmax 中会安全
                                # 且物理平滑地将权重归零，完全杜绝分母为0和 NaN 溢出，保持生成的高流畅性。
...
print(f"\nSuccessfully identified {len(model.config.hal_attention_heads)} custom hallucination heads (SAH & VSAH) based on your Red-Blue mapping matrix.")
